...
def get_case_insensitive_column(df, *names):
    """Return the first matching column name (case/space-insensitive) from df.columns."""
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        if 'EAN Codes' in sheet_names:
            df = pd.read_excel(uploaded_file, sheet_name='EAN Codes')
            df.dropna(how='all', inplace=True)
            df.columns = df.columns.str.strip()
            st.write(f"Columns in 'EAN Codes' sheet: {list(df.columns)}")
            # Find column names flexibly
            ean_col = get_case_insensitive_column(df, 'EAN', 'Ean Codes', 'EAN CODES')
            packed_col = get_case_insensitive_column(df, 'Packed')
            po_col = get_case_insensitive_column(df, 'PO')
            ordered_col = get_case_insensitive_column(df, 'Ordered')
            # Build up output columns based on what's there
            columns_to_use = []
            col_rename = {}
            if po_col:
                columns_to_use.append(po_col)
                col_rename[po_col] = "PO"
            if ean_col:
                columns_to_use.append(ean_col)
                col_rename[ean_col] = "EAN CODES"
            if ordered_col:
                columns_to_use.append(ordered_col)
                col_rename[ordered_col] = "ORDERED"
            if packed_col:
                columns_to_use.append(packed_col)
                col_rename[packed_col] = "PACKED"
            if ean_col and packed_col:
                df_filtered = df[columns_to_use].copy()
                # Remove rows with empty EAN or Packed
                df_filtered = df_filtered[df_filtered[ean_col].notnull() & df_filtered[packed_col].notnull()]
                ...
                st.success(f"Processed {len(df_filtered)} valid rows from 'EAN Codes' sheet.")
            else:
                st.error("No valid EAN or Packed column found in 'EAN Codes' sheet.")

        else:
            st.info(f"No 'EAN Codes' sheet found in file {uploaded_file.name}. Skipped for now.")
